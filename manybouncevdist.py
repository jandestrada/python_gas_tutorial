...
#set up velocity distribution histogram
velocity_dist=ghistogram(bins=arange(0,2*maxv,maxv/5),average=1,accumulate=1)
#set up expected velocity distribution curve
expected_distribution=gcurve(color=color.green)
#plot expected velocity distribution
for v in arange(0,2*maxv,maxv/20):
    expected_distribution.plot(pos=(v,0.3*no_particles*exp(-v**2/maxv**2*3/2)))
...
while (1==1): 
    ...
    #set up velocity list for plot
    vlist=[]
    for ball in balls:
        vlist.append(abs(ball.velocity.x))
    velocity_dist.plot(data=vlist)#use vlist data to plot velocity distribution
